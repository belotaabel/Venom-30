...
function getRequiredChannels(): RequiredChannel[] {
  return (process.env["TELEGRAM_REQUIRED_CHANNELS"]?.trim() || "@janoEarn,@janoEarn2").split(",").map((entry) => entry.trim()).filter(Boolean).map((entry) => {
